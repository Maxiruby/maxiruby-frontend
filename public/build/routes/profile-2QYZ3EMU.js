...
if (import.meta) {
  ...
  import.meta.hot.lastModified = "1693767580000";
}
var meta = () => {
  return [{
...
function Profile() {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      ...
      const response = fetch("http://maxiruby.com/api/users/auth/update", requestOptions);
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      const response = fetch("http://maxiruby.com/api/users/auth/resetpassword", requestOptions);
      const data = await (await response).json();
      setSuccess("Profile updated successfully");
      setTimeout(() => {
        setSuccess("");
...
//# sourceMappingURL=/build/routes/profile-2QYZ3EMU.js.map
